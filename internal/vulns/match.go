...
type Match struct {
	...
	EPSSPercent float64 `json:"epss_percentile,omitempty"`
...
type Fingerprint struct {
	...
	Version    string `json:"version,omitempty"`
	CPE        string `json:"cpe,omitempty"`
	VirtualCPE string `json:"virtual_cpe,omitempty"`
...
var cpeProducts = map[string]cpeProduct{
	...
	"bigip":         {"f5", "big-ip_access_policy_manager"},
	"coldfusion":    {"adobe", "coldfusion"},
	"confluence":    {"atlassian", "confluence"},
	"citrix":        {"citrix", "application_delivery_controller"},
...
func fingerprintsFromDetections(host string, port int, evidence, source string, detections []detected) []Fingerprint {
	...
	for _, d := range detections {
		...
		cpe, virtual := cpeFor(d.product, d.raw)
...
func FingerprintBanner(host string, port int, banner string) []Fingerprint {
	return fingerprintsFromDetections(host, port, banner, "banner", fromBanner(banner))
}

func FingerprintHTTP(host string, port int, h http.Header, body string) []Fingerprint {
	if h == nil {
		return nil
	}
	...
	return fingerprintsFromDetections(host, port, server, "http", detections)
}

func MatchesForFingerprints(fps []Fingerprint) []Match {
	var out []Match
	for _, fp := range fps {
		d := detected{
			product:      fp.Product,
			ver:          parseVersion(fp.Version),
			raw:          fp.Version,
			presenceOnly: fp.Version == "",
		}
		out = append(out, buildMatches(fp.Host, fp.Port, fp.Evidence, []detected{d})...)
	}
	return out
}
...
func (m Match) ToFinding() finding.Finding {
	...
	} else if m.Source == "nvd-live" {
		reason = "Detected service CPE/version matches NVD vulnerable configuration data for " + m.CVE
	}

	evidence := m.Banner
	if m.Product != "" {
		evidence = strings.TrimSpace(evidence + " product=" + m.Product)
	}
	if m.Version != "" {
		evidence = strings.TrimSpace(evidence + " version=" + m.Version)
	}
	if m.CPE != "" {
		evidence = strings.TrimSpace(evidence + " cpe=" + m.CPE)
	}
	if m.KEV {
		reason += " and CISA KEV marks it as known exploited"
	}
