...
func isHTTPService(port portscan.Result) bool {
	if httpPorts[port.Port] {
		return true
	}
	service := strings.ToLower(port.Service + " " + port.Banner)
	return strings.Contains(service, "http") || strings.Contains(service, "ssl/http") || strings.Contains(service, "https")
}

func schemeFor(port portscan.Result) string {
	service := strings.ToLower(port.Service + " " + port.Banner)
	if strings.Contains(service, "https") || strings.Contains(service, "ssl/http") || strings.Contains(service, "ssl") {
		return "https"
	}
	switch port.Port {
...
func detectTech(h http.Header, body string) []string {
	...
	if strings.Contains(b, "wp-content") || strings.Contains(b, "wordpress") {
		add("WordPress")
	}
	...
	if strings.Contains(b, "react") {
		add("React")
	}
	if strings.Contains(b, "vue.js") {
		add("Vue.js")
	}
	if strings.Contains(b, "angular") {
		add("Angular")
	}
	if strings.Contains(b, "jquery") {
		add("jQuery")
	}
	if strings.Contains(b, "laravel") {
		add("Laravel")
	}
	if strings.Contains(b, "django") {
		add("Django")
	}
	if strings.Contains(b, "fastapi") {
		add("FastAPI")
	}
