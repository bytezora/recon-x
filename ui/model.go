...
func New(target string) Model {
	...
	return Model{
		...
		steps: []step{
			...
			{name: "xss", desc: "reflected payload context check"},
			{name: "ssrf", desc: "metadata + loopback indicators"},
			{name: "lfi", desc: "path traversal signatures"},
			{name: "hostheader", desc: "host override injection"},
			{name: "jwt", desc: "token algorithm + claim audit"},
			{name: "wayback", desc: "historical endpoint mining"},
			{name: "shodan", desc: "passive host enrichment"},
			{name: "xxe", desc: "xml external entity probe"},
			{name: "cmdi", desc: "command injection indicators"},
		},
		spinner: sp,
		width:   80,
	}
